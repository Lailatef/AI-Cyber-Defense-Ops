...
16 rules in `./rules/` (13 from Module 4, 2 added in Module 5, 1 added 2026-07-18 as a bug fix — see caveat below), achieving **11/11 coverage** of the curated Credential Access technique subset in `mappings/attack_techniques.json` (not the full MITRE ATT&CK tactic — see caveat below). None of the three additions changes that 11/11 figure: `lsass_minidump` and `brute_force_single_account` are each a second rule for a technique (T1003.001 and T1110 respectively) that was already covered, and `azure_app_ropc_authentication` maps to a technique (T1078) that isn't in the curated mapping at all.
...
| `brute_force` | T1110 (password-spraying angle: one source IP against >10 accounts) | Loads/parses cleanly and passes `validate-rule.py` in full. Originally had an invalid Sigma condition (two aggregations joined by `or` in one clause — not valid Sigma grammar) and never actually evaluated despite appearing tag-covered; fixed 2026-07-18 — see caveat below. Not yet empirically validated against real brute-force EVTX telemetry (no matching sample in `samples/`). |
| `brute_force_single_account` | T1110 (distributed brute-force angle: one account targeted from >10 source IPs), added 2026-07-18 | Companion rule to `brute_force`, split out because Sigma only allows one aggregation function per condition. Loads/parses cleanly and passes `validate-rule.py` in full. Not yet empirically validated against real telemetry. |
...
| `lsass_minidump` | T1003.001 (second angle: `comsvcs.dll` MiniDump / ProcDump process-creation, companion to `lsass_memory_access`'s process-access angle) | `status: experimental`. Originally had a duplicate `CommandLine|contains` YAML key that caused it to fail loading entirely; fixed 2026-07-18 using the `|contains|all` list pattern already used elsewhere in the same file (`selection_procdump`). Now loads/parses cleanly and passes `validate-rule.py` in full (test-case comment added) — still not run against real EVTX telemetry (no MiniDump/ProcDump sample in `samples/`). |
...
**Honest caveat:** "Covered" means a rule exists and loads correctly in Hayabusa, not that it's been validated against real attack telemetry. Only `lsass_memory_access`, `dcsync`, and `t1187` have been confirmed against actual EVTX samples exercising that specific technique. The rest — including all three rules added after Module 4 — are reviewed, syntactically correct starter rules — good starting points, not production-hardened detections. Also, "11/11" reflects full coverage of the **11 techniques we chose to map** in `mappings/attack_techniques.json`, a hand-picked subset — not the complete MITRE ATT&CK Credential Access tactic, which has more sub-techniques than we've mapped here.

**Two rule-loading bugs found and fixed 2026-07-18, via a different module.** Both `brute_force.yml` and `lsass_minidump.yml` were previously broken in a way this module's own review missed: `brute_force.yml`'s `condition:` OR'd two aggregation expressions in one clause, which isn't valid Sigma grammar, so it never actually evaluated; `lsass_minidump.yml` had a duplicate `CommandLine|contains` YAML key, so it failed to load at all. Neither failure was obvious from Hayabusa's default/quiet output — its `Total detection rules: N` summary count doesn't reliably reflect how many rules are actually functional (it decrements for some parse failures but not others). Both bugs were caught by Module 6's `/investigate` slash command (`Module-6/siem-queries/`), which ran Hayabusa with `-v` against this exact `rule_dir` while writing an investigation note for a DCSync scan — not by anything in this module. Both are now fixed: `brute_force.yml` was split into itself plus a new `brute_force_single_account.yml` (Sigma allows only one aggregation per condition, so the OR'd pair had to become two rules), and `lsass_minidump.yml`'s duplicate key was merged into a `|contains|all` list. Hayabusa `-v` now reports 0 rule parsing errors and 16/16 rules loading (previously 13/15 functional). Full trace: `Module-6/siem-queries/investigations/2026-07-18_ca-dcsync-4662.md`. **Lesson: a rule count from a quiet/summary Hayabusa run is not sufficient confirmation that those rules are functional — spot-check with `-v`.**
...
├── CLAUDE.md
├── requirements.txt
├── rules/                  # 16 curated Sigma rules (our own, not the bundled Hayabusa set)
...
- **A rule count from Hayabusa's default/quiet output isn't proof the rules are functional** — two curated rules (`brute_force.yml`, `lsass_minidump.yml`) looked fine under normal output but were actually silently dropped (an invalid Sigma aggregation condition, and a duplicate YAML key, respectively). Neither was caught by this module's own review; both were found by Module 6's `/investigate` command running Hayabusa with `-v` against this `rule_dir`, months after the fact. Fixed 2026-07-18 — see `Module-6/siem-queries/investigations/2026-07-18_ca-dcsync-4662.md`. Worth remembering elsewhere: a summary count is not the same as verbose confirmation.
